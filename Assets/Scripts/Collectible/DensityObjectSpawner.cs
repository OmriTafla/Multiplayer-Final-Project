...
namespace Collectible
{
    public class DensityObjectSpawner : NetworkBehaviour
    {
        ...
        private const int MAX_NUM_FAILS_INITIAL_SPAWN = 1000;
        private const int MAX_TRACKED_OBJECTS = 1000;
        // [Networked, Capacity(MAX_TRACKED_OBJECTS)]
        private List<NetworkObject> SpawnedObjects { get; set; } = default;
        ...
        private IEnumerator SpawnObjectsCoroutine(float interval = -1f, int maxNumFails = -1)
        {
            ...
            while (!outOfAttempts(numFails))
            {
                ...
                {
                    SpawnedObjects.Add(Runner.Spawn(prefabToSpawn, spawnPosition));
                }
                
                yield return interval < 0 ?
                        null :
        ...
        {
            yield return SpawnObjectsCoroutine(maxNumFails: MAX_NUM_FAILS_INITIAL_SPAWN);
            
            yield return new WaitUntil(() => spawnCoroutineDone);
            
            yield return SpawnObjectsCoroutine(interval: spawnInterval);
        }
    }
}